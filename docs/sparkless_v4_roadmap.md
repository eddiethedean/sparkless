...
- [x] `robin-sparkless` version requirement documented (e.g. in `pyproject.toml` and README); install verified. (robin-sparkless>=0.6.0 documented in pyproject.toml and README; install verified.)
...
**Implementation note:** Phase 8 completed the release-readiness checklist. Robin-sparkless (>=0.6.0) is documented in pyproject.toml and README; install was verified with a minimal smoke test. CHANGELOG 4.0.0 and docs/release_notes_v4.md were drafted (breaking changes, migration, known limitations). README, migration_v3_to_v4, backend_selection, and guides (configuration.md, threading.md) were updated for v4 Robin-only; obsolete Polars/DuckDB references were clarified as v3-only. Version was set to 4.0.0 in pyproject.toml and sparkless/_version.py; release process is documented in docs/release_process.md. Tag and PyPI publish are deferred to maintainer discretion.
