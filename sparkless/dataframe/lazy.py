...
class LazyEvaluationEngine:
    ...
    def materialize(df: "DataFrame") -> "DataFrame":
        ...
        Sparkless v4 removed the materializer architecture. This method raises
        if called; use Robin DataFrame from sparkless.sql (e.g. spark.createDataFrame(...)).
        """
        raise NotImplementedError(
            "Sparkless v4 no longer uses a materializer. "
            "Use Robin DataFrame from sparkless.sql: "
            "spark = SparkSession.builder().getOrCreate(); df = spark.createDataFrame(...)"
        )
