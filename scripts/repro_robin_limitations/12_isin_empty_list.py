#!/usr/bin/env python3
"""
Verify: filter with isin([]) — empty list.
Sparkless fails with "Operation 'Operations: filter' is not supported".
This script tests direct Robin API to determine if it's Robin or Sparkless.

Run from repo root: python scripts/repro_robin_limitations/12_isin_empty_list.py
"""

from __future__ import annotations

import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent.parent
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

ROBIN_OK: list[str] = []
ROBIN_FAIL: list[str] = []
PYSPARK_OK: list[str] = []
PYSPARK_SKIP: list[str] = []


def run_robin() -> None:
    try:
        import robin_sparkless as rs
    except ImportError as e:
        ROBIN_FAIL.append(f"robin_sparkless not installed: {e}")
        return
    F = rs
    spark = F.SparkSession.builder().app_name("repro-isin-empty").get_or_create()
    data = [{"id": 1}, {"id": 2}, {"id": 3}]
    schema = [("id", "int")]
    create_df = getattr(spark, "create_dataframe_from_rows", None) or getattr(
        spark, "_create_dataframe_from_rows"
    )
    df = create_df(data, schema)
    try:
        out = df.filter(F.col("id").isin([])).collect()
        ROBIN_OK.append(f"filter(isin([])): {len(out)} rows (expected 0)")
    except Exception as e:
        ROBIN_FAIL.append(f"filter(isin([])): {type(e).__name__}: {e}")


def run_pyspark() -> None:
    try:
        from pyspark.sql import SparkSession
        from pyspark.sql import functions as F
    except ImportError:
        PYSPARK_SKIP.append("pyspark not installed")
        return
    spark = SparkSession.builder.master("local[1]").appName("repro").getOrCreate()
    try:
        df = spark.createDataFrame([{"id": 1}, {"id": 2}, {"id": 3}])
        out = df.filter(F.col("id").isin([])).collect()
        PYSPARK_OK.append(f"filter(isin([])): {len(out)} rows (expected 0)")
    except Exception as e:
        PYSPARK_OK.append(f"pyspark: {e}")
    finally:
        spark.stop()


def main() -> int:
    print("12_isin_empty_list: filter with col.isin([])")
    run_robin()
    run_pyspark()
    print("Robin:", "OK" if ROBIN_OK and not ROBIN_FAIL else "FAILED", ROBIN_OK or ROBIN_FAIL)
    print("PySpark:", "OK" if PYSPARK_OK else "SKIP", PYSPARK_OK or PYSPARK_SKIP)
    return 0 if ROBIN_FAIL else 1


if __name__ == "__main__":
    sys.exit(main())
