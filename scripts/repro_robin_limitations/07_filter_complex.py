#!/usr/bin/env python3
"""
Limitation: Filter with isin([]) or column-column comparison may raise
"Operation 'filter' is not supported" or RuntimeError.

Run from repo root: python scripts/repro_robin_limitations/07_filter_complex.py
"""
...
def run_robin() -> None:
    ...
    try:
        out = df.filter(F.col("a") > F.col("b")).collect()
        ROBIN_OK.append(f"filter(col('a')>col('b')): {len(out)} rows")
    except Exception as e:
        ROBIN_FAIL.append(f"filter(col('a')>col('b')): {type(e).__name__}: {e}")
...
def run_pyspark() -> None:
    ...
    try:
        df = spark.createDataFrame([{"a": 1, "b": 2}, {"a": 2, "b": 1}, {"a": 3, "b": 3}])
        out = df.filter(F.col("a") > F.col("b")).collect()
        PYSPARK_OK.append(f"filter(col('a')>col('b')): {len(out)} rows")
    except Exception as e:
...
def main() -> int:
    print("07_filter_complex: filter with column-column comparison (a > b)")
    run_robin()
    run_pyspark()
