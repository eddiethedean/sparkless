...
def check_version_numbers() -> List[Tuple[str, bool, str]]:
    ...
    for file_path, pattern in files_to_check:
        ...
        if matches:
            ...
            version_found = (
                matches[0] if isinstance(matches[0], str) else matches[0].split()[-1]
            )
                ...
                if pkg_major_minor == found_major_minor:
                    results.append(
                        (
                            file_path,
                            True,
                            f"Version {found_version} found (package: {package_version})",
                        )
                    )
                else:
                    results.append(
                        (
                            file_path,
                            False,
                            f"Version mismatch: found {found_version}, package {package_version}",
                        )
                    )
            else:
                results.append(
                    (file_path, True, f"Version pattern found: {matches[0]}")
                )
        else:
...
def check_feature_claims() -> List[Tuple[str, bool, str]]:
    ...
    try:
        ...
        func_count = len(
            [
                m
                for m in dir(Functions)
                if not m.startswith("_") and callable(getattr(Functions, m))
            ]
        )
        results.append(
            ("120+ functions", func_count >= 120, f"Found {func_count} functions")
        )
    ...
    try:
        ...
        method_count = len(
            [
                ...
                if not m.startswith("_") and callable(getattr(DataFrame, m))
            ]
        )
        results.append(
            (
                "70+ DataFrame methods",
                method_count >= 70,
                f"Found {method_count} methods",
            )
        )
    except Exception as e:
...
def check_links() -> List[Tuple[str, bool, str]]:
    ...
    for md_file in markdown_files:
        ...
        for link_text, link_path in links:
            ...
            if target.exists():
                results.append(
                    (f"{md_file.name}: {link_text}", True, f"Link to {link_path} works")
                )
            else:
                results.append(
                    (f"{md_file.name}: {link_text}", False, f"Broken link: {link_path}")
                )
...
def main() -> int:
    ...
    if len(link_results) > 20:
        ...
        failures_remaining = sum(
            1 for _, is_valid, _ in link_results[20:] if not is_valid
        )
