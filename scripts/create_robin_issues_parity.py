#!/usr/bin/env python3
"""
Create robin-sparkless GitHub issues for PySpark parity (from v4 skipped tests).

Run from repo root:
  python scripts/create_robin_issues_parity.py [--dry-run] [--issue 1|2|3|4|all]
...
ISSUES = [
    ...
    {
        "num": 2,
        "title": "[PySpark parity] groupBy + agg (sum, count) in logical plan",
        "body_file": DOCS / "robin_github_issue_groupby_agg_plan.md",
    },
    {
        "num": 3,
        "title": "[PySpark parity] between, power (**), and cast in logical plan",
        "body_file": DOCS / "robin_github_issue_between_power_cast_plan.md",
    },
    {
        "num": 4,
        "title": "[PySpark parity] row_number() over (partition by col) window",
        "body_file": DOCS / "robin_github_issue_window_row_number_plan.md",
    },
]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create robin-sparkless GitHub issues for PySpark parity"
    )
    parser.add_argument(
